...
<p>作函数图象最基本的方法是描点画图, 但是对于复杂的函数, 仍需借助函数的性质才能较准确地作出其图象. 通常前面已经结合函数图象定义了单调性和奇偶性, 而已知函数的单调性和奇偶性 (如事先判断对应的代数特征) 后能更准确地作出函数图象.分段函数的图象则需分段作出.
</p>
...
<p>先考虑函数 $y=f(x)$ 与 $y=f(x+1)$ 的图形. 因为点 $A(k,f(a))$ 满足 $y=f(x)$ (此时 $x=a$), 而点 $A'(a-1,f(a))$ 满足 $y=f(x+1)$ (此时 $x=a-1$), 且点 $A$ 向左平移一个单位长度可得点 $A'$, 所以将 $y=f(x)$ 图形上的所有点向左平移一个单位长度可得 $y=f(x+1)$ 的图形.
</p>
...
<myexample>
    <p>已知定义在 $\mathbf{R}$ 上的奇函数 $f(x)$ 满足 $f(x+4)=f(x)$ 且 $f(1)=1$, 求 $f(3)+f(4)+f(5)$ 的值.
    </p>
</myexample>
<mysolution>
...
<p>函数的图象用图形来表示函数的自变量和因变量之间的关系, 即表示其对应法则.
  函数图象最基本的画法是<strong>描点法</strong>.  对常见的函数, 如一次函数, 二次函数, 反比例函数, 幂函数, 指数函数, 对数函数, 三角函数, 需掌握它们图象的画法. 与函数的图象密切相关的函数性质有单调性和奇偶性.
</p>
<p>函数图象的第二种画法是利用\myindex{图象变换} (graph tranformation), 如要得到 $y=(x-1)^2$ 的图象, 
  只需把 $y=x^2$ 的图象向右平移 1 个单位长度. 设函数为 $y=f(x)$, $a>0$,
  常用的图象变换法则如下 (考虑变换前后对应点的坐标可知):
</p>
<p>向左平移 $a$ 个单位长度: $f(x)\rightarrow f(x+a)$;
</p>
<p>向右平移 $a$ 个单位长度: $f(x)\rightarrow f(x-a)$;
</p>
<p>向上平移 $a$ 个单位长度: $f(x)\rightarrow f(x)+a$;
</p>
<p>向下平移 $a$ 个单位长度: $f(x)\rightarrow f(x)-a$;
</p>
<p>关于 $y$ 轴对称: $f(x)\rightarrow f(-x)$;
  \mymarginpar{关于 $x=a$ 轴对称: $f(x)\rightarrow f(2a-x)$;\\
    关于 $y=b$ 轴对称: $f(x)\rightarrow 2b-f(x)$.}
</p>
<p>关于 $y$ 轴对称: $f(x)\rightarrow f(-x)$;
</p>
<p>关于 $x$ 轴对称: $f(x)\rightarrow -f(x)$;
</p>
<p>纵坐标不变, 横坐标变为原来的 $a$ 倍: $f(x)\rightarrow f\Big(\frac{x}a\Big)$;
</p>
<p>横坐标不变, 纵坐标变为原来的 $a$ 倍: $f(x)\rightarrow af(x)$.\\
...
<p>思考: $|f(x)|$ 与 $f(x)$, $f(|x|)$ 与 $f(x)$ 的图象关系分别是什么?
...
<p>函数图象的第三种画法是利用导数确定单调性和极值点, 再描点作图.
</p>
<p>\lianxi
  <myexercise>
    <p>函数 $y=|x+1|$ 的图象是\,?
...
<p>\draw[line width=0.5pt] (1,0)--(1,0.1);
      \draw (1,0) node[below] {$1$};
      \draw (2,0) node[below] {$2$};
      \draw (0,2) node[left] {$2$};
      \draw (0,1) node[left] {$1$};
...
<myexample>
<p>根据所给定义域, 画出函数 $y=x^2-2x+2$ 的图象.
</p>
<p>(1) $x\in \mathbf{R}$; \qquad
    (2) $x\in (-1,2]$;\qquad 
    (3) $x\in (-1,2]$ 且 $x\in\mathbf{Z}$.
  </p>
</myexample>
</p>
<p><mysolution>
...
<p><mysolution>
    <p>    $y=3-\frac2{x+2}$, 图象可由 $y=-\frac2x$ 的图象平移得到.
...
<p>(1) 求 $f(-3)$, $f\big(f(-3)\big)$;\qquad
    (2) 画出 $y=f(x)$ 的图象;
</p>
<p>(3) 若 $f(a)=\dfrac12$, 求 $a$ 的值.
  </p>
</myexercise>
</p>
<p><mysolution>
    <p>    (1) $f(-3)=2$, $f\big(f(-3)\big)=f(2)=4$.
</p>
<p>(2) 分段作出 $y=f(x)$ 的图象.
    \mymarginpar{\centering
    \begin{tikzpicture}[line cap=round,line join=round,scale=0.4]
...
<p>(3) 分 $a\leqslant 1$, $-1< a< 1$, $a\geqslant 1$ 解得, $a=-\frac92$ 或 $\pm\frac{\sqrt2}2$.
  </p>
</mysolution>
</p>
<p><myexercise>
